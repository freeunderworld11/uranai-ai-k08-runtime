...
| K 0度牡羊近辺 | 0度直後の全参照値、正規化・サイン内度数・0/360をまたぐ角距離を固定検証 | 本番受入・最終承認 |
| L 日付変更線付近 | Kiritimatiの年日付跨ぎ入力・全参照値、UTC日付誤り拒否を検証 | 本番受入・最終承認 |
...
| FIXED_NATAL_REGRESSION_SUITE | A〜Lの12分類の固定入力・native参照値はローカル照合済み。条件付き項目の認証と正式受入は未完了 |
