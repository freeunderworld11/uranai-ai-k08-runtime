...
export function withPlanetHouses(swe,result,latitude) {
  ...
  const positions=result.positions.map(p=>{
    const unavailable=reason=>({...p,house:null,house_status:'UNAVAILABLE',house_limitation:reason});
    ...
    return {...p,house:Math.floor(h.position),house_position:h.position,house_status:'VALID',house_method:'swe_house_pos',house_cusp_sensitive:null};
  });
