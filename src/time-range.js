...
export function calculateRangeWith(swe,gate) {
  ...
  for(let i=0;i<=intervals;i++) {
    const sample=calculateWith(swe,{jd_ut:i===intervals?jd[1]:jd[0]+(jd[1]-jd[0])*i/intervals},{skipHouses:true});
  ...
  return {input_echo:gate.input_echo,input_contract:gate.input_contract,range_contract:gate.range_contract,
    ...
    limitations:[...new Set(gate.endpoints.flatMap(e=>e.limitations)),'EXPLICIT_RANGE_NOT_VERIFIED_AGAINST_LOCAL_TIME','SAMPLED_SIGN_AGREEMENT_IS_NOT_PROOF','HOUSE_AND_ASPECT_RANGE_VALIDATION_PENDING']};
}
