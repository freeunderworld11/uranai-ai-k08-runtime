...
export async function calculateK02(source) {
  ...
  try {
    ...
    return {...withAspects(withPlanetDetails(result)),input_contract:'K02_TO_K08_ASTRO_TIME_GEO_v1',...audit,derived_runtime_value:{JULIAN_DAY_UT:jd},result_status:gate.time_status==='CONDITIONAL'&&result.result_status==='COMPLETE'?'PARTIAL':result.result_status};
  } catch(error) {
    if(error instanceof CalculationError)throw error;
    throw new CalculationError('RUNTIME_UNAVAILABLE',503);
  } finally {swe?.dispose();}
...
export async function calculate(input) {
  ...
  try {
    swe = await createSwissEph();
    swe.mountEphemeris({'sepl_18.se1':planets,'semo_18.se1':moon,'seas_18.se1':asteroids});
    return withAspects(withPlanetDetails(calculateWith(swe,input)));
  } catch (error) {
    if (error instanceof CalculationError) throw error;
    throw new CalculationError('RUNTIME_UNAVAILABLE',503);
  } finally { swe?.dispose(); }
