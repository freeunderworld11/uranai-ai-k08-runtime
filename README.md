...
[夏時間ケースの条件](docs/CASE_B.md)と参照値を追加しました。ロンドン・2000年7月1日12時の入力を照合し、冬時間offsetの誤用を拒否することも確認しました。全74テスト成功。K19全体の本番承認は保留です。


## CASE_C固定回帰テスト

[南半球ケースの検証条件](docs/CASE_C.md)と固定参照値を追加しました。シドニー・2000年7月1日12時の入力で天体・角度・カスプ・アスペクトを照合し、全75テストが成功しました。本番承認は引き続き保留です。


## CASE_D固定回帰テスト

[西経地域ケースの検証条件](docs/CASE_D.md)と固定参照値を追加しました。ニューヨーク・2000年1月1日12時の入力で天体・角度・カスプ・アスペクトを照合し、全76テストが成功しました。本番承認は引き続き保留です。


## CASE_E固定回帰テスト

[歴史的時差の検証条件](docs/CASE_E.md)と変更前後2入力を追加しました。パリ1911年の秒単位offsetを保持し、歴史的信頼性は条件付きのまま検査します。全79テスト成功。本番承認は保留です。


## CASE_F固定回帰テスト

[太陽サイン境界の条件](docs/CASE_F.md)と前後2入力を追加しました。1秒違いの魚座・牡羊座の判定と境界感度、全出生図参照値を照合し、全81テストが成功しました。本番承認は保留です。
...
[高緯度失敗ケース](docs/CASE_J.md)の固定期待値を追加しました。正常な10天体・アスペクトを保持し、代替カスプと所属ハウスを不採用、ASC/MCは条件付きとすることを検証しました。全87テスト成功。本番承認は保留です。


## CASE_K固定回帰テスト

[0度牡羊近辺の条件](docs/CASE_K.md)と固定参照値を追加しました。正規化・サイン内度数・0/360をまたぐ角距離と全出生図を照合し、全88テストが成功しました。本番承認は保留です。


## CASE_L固定回帰テスト

[日付変更線付近の条件](docs/CASE_L.md)を追加しました。現地2000年1月1日とUTC1999年12月31日の接続、日付誤り拒否を確認し、全90テストが成功しました。A〜Lの12分類がローカル照合済みですが、K19正式受入・本番承認は未完了です。
